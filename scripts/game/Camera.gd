...
func _input(event):
	if event is InputEventMouseMotion and Flux.get_setting("game", "spin"):
		yaw = fmod(yaw + event.relative.y * Flux.get_setting("cursor", "sensitivity") / 250.0, 360) 
		pitch = max(min(pitch + event.relative.x * Flux.get_setting("cursor", "sensitivity") / 250.0, 90),-90)
